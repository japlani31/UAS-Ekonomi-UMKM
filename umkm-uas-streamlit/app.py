# -*- coding: utf-8 -*-
"""
UAS Ekonomi UMKM (Kelas K3 & K4) - Universitas Muhammadiyah Metro
Ardiansyah Japlani, S.E., M.B.A. - FEB UM Metro.

Aplikasi fokus tunggal: Ujian Akhir Semester Ekonomi UMKM (2 kelas: K3 & K4) dengan pengaman anti-contek,
...
st.set_page_config(page_title="UAS Ekonomi UMKM - UM Metro",
...
def ambil_soal_uas(npm, kelas):
    """
    Mengambil paket soal unik untuk satu NPM dari bank kelasnya (K3 atau K4).
    - Memilih JUMLAH_SOAL_PER_MAHASISWA soal acak dari bank kelas.
...
def buat_kartu_hasil(nama, npm, nilai, benar, total, waktu_str):
    ...
    d.text((24, 20), "BUKTI PENGERJAAN UAS EKONOMI UMKM", font=font(20, True), fill="white")
...
# ============================================================
def halaman_uas():
    st.header("🎓 Ujian Akhir Semester (UAS) Ekonomi UMKM")
    _mesin_ujian(
        judul="UAS",
        jenis="uas",
...
# ============================================================
def _mesin_ujian(judul, jenis, durasi_menit):
    ...
    if not st.session_state.get(ns + "mulai"):
        st.markdown(
            f"""
            **Ketentuan {judul}:**
            - Jumlah soal: **{uas_soal.JUMLAH_SOAL_PER_MAHASISWA}**, setiap mahasiswa berbeda.
            - Waktu: **{durasi_menit} menit**, tidak ada waktu tambahan.
            - Pilih **kelas** dengan benar. Soal dan penyimpanan nilai mengikuti kelas.
        ...
        kelas = st.selectbox("Kelas", uas_soal.KELAS_TERSEDIA, index=None,
                             placeholder="Pilih kelas Anda", key=ns + "kelas_in")
        ...
        if st.button(f"Mulai {judul}", type="primary", key=ns + "btn_mulai"):
            ...
            if not setuju:
                ...
                return
            # Cek satu NPM satu kesempatan pada kelas tsb (bila Sheet tersambung)
            if _SHEETS_ADA and sheets.npm_sudah_mengerjakan(npm, kelas):
                st.error(f"NPM ini sudah pernah mengerjakan di kelas {kelas}. Ujian tidak dapat diulang.")
                return
            st.session_state[ns + "mulai"] = True
            st.session_state[ns + "kelas"] = kelas
            st.session_state[ns + "nama"] = nama.strip()
            st.session_state[ns + "npm"] = npm.strip()
            st.session_state[ns + "paket"] = ambil_soal_uas(npm.strip(), kelas)
    ...
    st.caption(f"Kelas {st.session_state[ns+'kelas']} · {st.session_state[ns+'nama']} · NPM {st.session_state[ns+'npm']}")
...
def _finalisasi(ns, jenis, otomatis):
    ...
    if _SHEETS_ADA:
        ...
        status, pesan = sheets.simpan_hasil(
            ...
            nilai, benar, total, st.session_state[ns + "kelas"],
            pindah_tab=pindah, durasi_detik=durasi, catatan=catatan,
        )
